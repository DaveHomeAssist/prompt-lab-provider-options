...
- `Build` is also not a separate primary view
...
| `Experiments` | sets `primaryView=runs` and defaults `runsView=compare` |
...
| Visible label | What it actually does |
|---|---|
| `Build` | sets `primaryView=create`, `workspaceView=composer` |
| `Notebook` | sets `primaryView=notebook` |

This means `Build` and `Notebook` are treated as utility-mode buttons rather than part of the main 3-button workspace cluster.
...
The app may show layout options:

- `Focus`
- `Dual Pane`

These are backed by `editorLayout` / `effectiveEditorLayout`.
...
### When active section is Experiments
...
- for example:
  - visible header says `Experiments`
  - command palette includes both `Go to Experiments` and `Go to Experiment History`
  - visible header says `Build`
  - command palette says `Open Build Utility`
...
2. `Build` is visible as a utility button, but it is actually `workspaceView=composer`.
3. `Notebook` is a true `primaryView`, but it is visually grouped with utility buttons rather than with the main workspace trio.
...
- any attempt to unify or simplify the Create / Library / Build / Notebook model
